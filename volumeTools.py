	...
	for i in range(num_samp):
		for j in range(num_samp):
			for k in range(num_samp):
				...
				mj = num_samp-1-j
				mk = num_samp-1-k
				#pairs = np.array([ idata[i,j,k], idata[mi,mj,mk], idata[mi,j,mk], idata[i,mj,k] ]) #P1211 , idata[i,j,mk], idata[mi,j,k], idata[mi,mj,k], idata[i,mj,mk]])
				pairs = np.array([ idata[i,j,k], idata[mi,mj,k], idata[mj,i,k], idata[j,mi,k] ])
				ori = pairs.copy()
