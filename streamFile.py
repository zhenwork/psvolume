...
class streamFile:
			...
			if istart or line==len(self.content)-1:
				...
				for row in single:
					...
					for idx in range(len(row)):
						if row[idx]!=-1: 
							if self.label.map[idx]==-1: continue
							else:
								tmpRow = tmpLabel[self.label.map[idx]][:]
								if row[self.label.map[idx]]==-1: row=None; break
								if max([x for x in tmpRow if x<row[idx]])==row[self.label.map[idx]]: continue
								else: row=None; break
						else:
	...
	def get_eventList(self, fcxi):
		try: 
			...
			f.close()
			return  ievent
		except: raise Exception('### no cxi file ###')
	...
	def get_realPeak(self):
		...
		if not self.label.status: 
			...
			self.label.status = True
		else: tmpStream.label.index = self.backup.index
	...
	def get_info(self, alg=2):
		...
		for i in range(len(tmpStream.label.index)):
			...
			if data[i,3]<-15: data[i,3] = tmpStream.label.index[i,4]-tmpStream.label.index[i,3]-2
			else: raise Exception('### error occurs for nPeaks ###')
			if data[i,4]<-15: data[i,4] = tmpStream.label.index[i,7]-tmpStream.label.index[i,6]-2
			else: raise Exception('### error occurs for nReflection ###')
	...
	def save_label(self, fsave=None):
		...
		if os.path.exists(fsave): 
			f = h5py.File(fsave, 'r+')
			try: f.__delitem__('label')
			except: pass
		else: f = h5py.File(fsave, 'w')
